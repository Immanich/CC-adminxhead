    ...
    public function approveService($serviceId)
{
    ...
    // Ensure service is not already approved
    if ($service->status === 'approved') {
        return redirect()->route('pending.services')->with('error', "Service '{$service->service_name}' is already approved.");
    }

    // Update the service status to 'approved'
